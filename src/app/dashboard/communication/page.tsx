  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${unread} message${unread > 1 ? "s" : ""} non lu${unread > 1 ? "s" : ""}`}
        actions={<Button size="sm" onClick={() => setCompose(true)}><Plus className="w-3.5 h-3.5" /> Nouveau message</Button>}
      />
      ...
      <div className="flex gap-4 h-[calc(100vh-220px)] min-h-[480px]">
        ...
        <div className="w-[300px] shrink-0 flex flex-col gap-2">
          ...
          <div className="flex-1 overflow-y-auto space-y-0.5">
            ...
            {filtered.length === 0 && (
              <div className="py-12 text-center text-[12px] text-[var(--ink-4)]">Aucun message</div>
            )}
          </div>
        </div>
          ...
          {compose ? (
            <Card className="h-full flex flex-col">
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-[var(--line)] shrink-0">
                <h2 className="font-semibold text-[13px] text-[var(--ink)]">Nouveau message</h2>
                <button onClick={() => setCompose(false)} className="text-[var(--ink-4)] hover:text-[var(--ink)] text-[11px]">Annuler</button>
              </div>
              <div className="flex-1 p-5 space-y-3">
                <div>
                  <label className="text-[10.5px] font-bold uppercase tracking-wide text-[var(--ink-4)] block mb-1">Destinataire</label>
                  <Input placeholder="Nom ou email…" />
                </div>
                <div>
                  <label className="text-[10.5px] font-bold uppercase tracking-wide text-[var(--ink-4)] block mb-1">Objet</label>
                  <Input placeholder="Sujet du message…" />
                </div>
                <div className="flex-1">
                  <label className="text-[10.5px] font-bold uppercase tracking-wide text-[var(--ink-4)] block mb-1">Message</label>
                  <Textarea placeholder="Rédigez votre message…" className="resize-none h-48" />
                </div>
              </div>
              <div className="flex gap-2 px-5 pb-4 shrink-0">
                <Button className="gap-1.5"><Send className="w-3.5 h-3.5" /> Envoyer</Button>
                <Button variant="ghost">Sauvegarder brouillon</Button>
              </div>
            </Card>
          ) : selected ? (
            <Card className="h-full flex flex-col">
            ...
            <Card className="h-full flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-[13px] text-[var(--ink-4)]">Sélectionnez un message pour le lire</p>
              </div>
            </Card>
          )}
        </div>
